...
var init_harness = __esm({
  "src/harness.ts"() {
    "use strict";
    DEFAULTS = { warmup: 8, iterations: 40, inner: 1 };
  }
});
...
function compareResults(baseline2, current, options = {}) {
  const band = options.band ?? 0.1;
  const baseByName = /* @__PURE__ */ new Map();
  for (const b of baseline2) baseByName.set(b.name, b);
  const out = [];
...
function writeSuite(fileName2, suite2) {
  const dir = resultsDir();
  fs.mkdirSync(dir, { recursive: true });
  const full = path.join(dir, fileName2);
  fs.writeFileSync(full, JSON.stringify(suite2, null, 2) + "\n", "utf8");
  return full;
}
function readSuite(fileName2) {
  const full = path.join(resultsDir(), fileName2);
...
function formatEnvironment(suite2) {
  const e = suite2.environment;
  return [
    `node       ${e.node}`,
...
var init_reactivity = __esm({
  "src/suites/reactivity.ts"() {
    ...
    init_apps();
    init_dom_env();
    SUBSCRIBERS = [1, 10, 100, 1e3];
    CHAIN_DEPTHS = [10, 100, 500];
  }
});
...
__export(index_exports, {
  SUITES: () => SUITES,
  bench: () => bench,
  buildAttrApp: () => buildAttrApp,
  buildConditionalApp: () => buildConditionalApp,
  buildFlatApp: () => buildFlatApp,
  buildListApp: () => buildListApp,
  buildReactiveApp: () => buildReactiveApp,
  compareResults: () => compareResults,
  ...
  installHappyDom: () => installHappyDom,
  makeItems: () => makeItems,
  readSuite: () => readSuite,
  resultsDir: () => resultsDir,
...
function runAllSuites(options = {}) {
  ...
  for (const suite2 of SUITES) {
    if (filter2 !== void 0 && filter2.length > 0 && !filter2.includes(suite2.name)) continue;
    options.onSuite?.(suite2.name);
    results.push(...suite2.run());
  }
...
init_dom_env();
installHappyDom();
var { runAllSuites: runAllSuites2 } = await Promise.resolve().then(() => (init_index(), index_exports));
var { compareResults: compareResults2 } = await Promise.resolve().then(() => (init_compare(), compare_exports));
var {
  writeSuite: writeSuite2,
  readSuite: readSuite2,
  formatResults: formatResults2,
  formatComparison: formatComparison2,
  formatEnvironment: formatEnvironment2
} = await Promise.resolve().then(() => (init_report(), report_exports));
function parseArgs(argv) {
  let baseline2 = false;
  let filter2 = [];
  for (const arg of argv) {
    if (arg === "--baseline") baseline2 = true;
    else if (arg.startsWith("--suite=")) {
      filter2 = arg.slice("--suite=".length).split(",").map((s) => s.trim()).filter((s) => s.length > 0);
    }
  }
  return { baseline: baseline2, filter: filter2 };
}
var { baseline, filter } = parseArgs(process.argv.slice(2));
var suite = runAllSuites2({
  filter,
  onSuite: (name) => process.stderr.write(`  running ${name}\u2026
`)
});
process.stdout.write("\nEnvironment\n");
process.stdout.write(formatEnvironment2(suite) + "\n\n");
process.stdout.write(formatResults2(suite.results) + "\n\n");
var fileName = baseline ? "baseline.json" : "current.json";
var written = writeSuite2(fileName, suite);
process.stdout.write(`Wrote ${suite.results.length} results \u2192 ${written}
`);
if (!baseline) {
  const base = readSuite2("baseline.json");
  if (base !== null) {
    process.stdout.write("\nComparison vs baseline.json\n");
    const comparisons = compareResults2(base.results, suite.results);
    process.stdout.write(formatComparison2(comparisons) + "\n");
  } else {
