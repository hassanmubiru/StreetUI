...
function buildUserDetail(page, deps, ctx) {
  ...
  page.section("user-detail", (s) => {
    s.heading(derived2(() => i18n.translate("detailHeading", { id })), { id: "detail-heading", level: 2 });
    s.errorBoundary("detail-boundary", (b) => {
      b.when(userDetail.loading, (l) => l.text(i18n.t("loading"), { id: "detail-loading" }));
      b.when(
        derived2(() => userDetail.data.get() !== void 0),
        (d) => d.text(derived2(() => userDetail.data.get()?.name ?? ""), { id: "detail-name" })
      );
    }, {
